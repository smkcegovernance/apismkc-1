...
{
    /// <summary>
    /// Consent Document Controller for Deposit Manager
    /// Provides common API endpoint for downloading bank consent documents
    ...
    public class ConsentDocumentController : ApiController
    {
        ...
        /// GET: /api/deposits/consent/health
        /// </summary>
        [HttpGet]
        [Route("health")]
        [AllowAnonymous]
        ...
        {
            FtpLogger.LogInfo("=== CONSENT_CONTROLLER - HealthCheck - Request Received ===");
            ...
            var response = new
            {
                ...
                storageType = System.Configuration.ConfigurationManager.AppSettings["Storage_Type"] ?? "ftp",
                logFilePath = FtpLogger.GetLogFilePath(),
                authenticationRequired = false
            };
            
            FtpLogger.LogInfo("CONSENT_CONTROLLER - HealthCheck - Success");
            
            return Request.CreateResponse(HttpStatusCode.OK, response);
        }

        /// <summary>
        /// Download bank consent document by requirement ID and quote ID
        /// GET: /api/deposits/consent/downloadconsent
        ...
        /// <param name="fileName">Consent file name (optional - if known from quote data)</param>
        /// <returns>Binary file download or JSON response with base64 data</returns>
        [HttpGet]
        [Route("downloadconsent")]
        [AllowAnonymous]
        public HttpResponseMessage DownloadConsentDocument(string requirementId, string quoteId = null, string bankId = null, string fileName = null)
        {
            ...
            try
            {
                // Log request start
                FtpLogger.LogInfo("================================================================================");
                FtpLogger.LogInfo(string.Format("=== CONSENT DOWNLOAD REQUEST START - RequestId: {0} ===", requestId));
                FtpLogger.LogInfo("================================================================================");
                ...
                FtpLogger.LogInfo(string.Format("  QuoteId: {0}", quoteId ?? "(null)"));
                ...
                LogRequest(string.Format("  QuoteId: {0}", quoteId ?? "(null)"), true);
                ...
                // If fileName is not provided, we need quoteId to look it up
                if (string.IsNullOrWhiteSpace(fileName) && string.IsNullOrWhiteSpace(quoteId))
                {
                    FtpLogger.LogError(string.Format("VALIDATION FAILED - Missing both fileName and quoteId [RequestId: {0}]", requestId));
                    LogRequest("DownloadConsentDocument - Missing both fileName and quoteId", false);
                    return Request.CreateResponse(HttpStatusCode.BadRequest, 
                        new ApiResponse 
                        { 
                            Success = false, 
                            Message = "Either fileName or quoteId must be provided",
                            Error = "INVALID_PARAMETER"
                        });
                }
                ...
                // If fileName is not provided, try to look it up using quoteId
                // Note: This would require additional repository access to get quote details
                // For now, we assume fileName is provided or will be added as enhancement
                if (string.IsNullOrWhiteSpace(fileName))
                {
                    FtpLogger.LogWarning(string.Format("FILENAME LOOKUP NOT IMPLEMENTED - fileName required [RequestId: {0}]", requestId));
                    LogRequest("DownloadConsentDocument - fileName lookup not yet implemented", false);
                    return Request.CreateResponse(HttpStatusCode.BadRequest, 
                        new ApiResponse 
                        { 
                            Success = false, 
                            Message = "fileName parameter is required. Please provide the consent file name.",
                            Error = "FILENAME_REQUIRED",
                            Data = new 
                            { 
                                hint = "You can get the fileName from the quote details endpoint",
                                example = "?requirementId=REQ001&bankId=BANK001&fileName=consent_document.pdf"
                            }
                        });
                }

                FtpLogger.LogStep(2, "Preparing to download file from storage", true);
                ...
                FtpLogger.LogStep(3, "Calling FTP Storage Service", true, 
                    string.Format("Downloading: {0} for {1}/{2}", fileName, requirementId, bankId));
                
                var base64Content = _ftpStorage.DownloadConsentDocument(fileName, requirementId, bankId);
                
                if (base64Content == null)
                {
                    FtpLogger.LogError(string.Format("FILE NOT FOUND - {0} [RequestId: {1}]", fileName, requestId));
                    FtpLogger.LogError(string.Format("  Expected path: RequirementId={0}, BankId={1}, FileName={2}", 
                        requirementId, bankId, fileName));
                    
                    LogRequest(string.Format("DownloadConsentDocument - File not found: {0}", fileName), false);
                    LogRequest(string.Format("  Expected location: \\\\SERVER\\SHARE\\{0}\\{1}\\{2}", requirementId, bankId, fileName), false);
                ...
                FtpLogger.LogStep(3, "FTP Storage Service completed", true, 
                ...
                if (returnBinary)
                {
                    ...
                    FtpLogger.LogInfo("=== CONSENT DOWNLOAD REQUEST COMPLETED SUCCESSFULLY (BINARY) ===");
                    FtpLogger.LogInfo(string.Format("RequestId: {0}", requestId));
                    FtpLogger.LogInfo("================================================================================");
                    FtpLogger.LogInfo("");
                ...
                else
                {
                    ...
                    FtpLogger.LogInfo("=== CONSENT DOWNLOAD REQUEST COMPLETED SUCCESSFULLY (JSON) ===");
                    FtpLogger.LogInfo(string.Format("RequestId: {0}", requestId));
                    FtpLogger.LogInfo("================================================================================");
                    FtpLogger.LogInfo("");
            ...
            {
                FtpLogger.LogError("================================================================================");
                FtpLogger.LogError(string.Format("=== CONSENT DOWNLOAD REQUEST FAILED - RequestId: {0} ===", requestId));
                FtpLogger.LogError("================================================================================");
        ...
        public HttpResponseMessage GetConsentDocumentInfo(string requirementId, string bankId, string fileName)
        {
            ...
            try
            {
                // Log request start
                FtpLogger.LogInfo("================================================================================");
                FtpLogger.LogInfo(string.Format("=== CONSENT INFO REQUEST START - RequestId: {0} ===", requestId));
                FtpLogger.LogInfo("================================================================================");
                FtpLogger.LogInfo(string.Format("Client IP: {0}", clientIp));
                FtpLogger.LogInfo(string.Format("RequestId: {0}", requestId));
                FtpLogger.LogInfo(string.Format("Timestamp: {0:yyyy-MM-dd HH:mm:ss.fff} UTC", DateTime.UtcNow));
                ...
                FtpLogger.LogStep(2, "Checking file existence", true, 
                ...
                if (base64Content == null)
                {
                    ...
                    FtpLogger.LogError(string.Format("  Expected path: RequirementId={0}, BankId={1}, FileName={2}", 
                        requirementId, bankId, fileName));
                    FtpLogger.LogInfo("=== CONSENT INFO REQUEST COMPLETED (FILE NOT FOUND) ===");
                    FtpLogger.LogInfo(string.Format("RequestId: {0}", requestId));
                    FtpLogger.LogInfo("================================================================================");
                    FtpLogger.LogInfo("");
                    
                    LogRequest(string.Format("GetConsentDocumentInfo - File not found: {0}", fileName), false);
                    LogRequest(string.Format("  Expected path: \\\\SERVER\\SHARE\\{0}\\{1}\\{2}", requirementId, bankId, fileName), false);
                    return Request.CreateResponse(HttpStatusCode.NotFound, 
                        new ApiResponse 
                        { 
                            Success = false, 
                ...
                var response = new ApiResponse
                {
                    ...
                    Data = new
                    {
                        ...
                        bankId = bankId,
                        exists = true,
                        downloadUrl = string.Format("/api/deposits/consent/downloadconsent?requirementId={0}&bankId={1}&fileName={2}", 
                            requirementId, bankId, fileName)
                    }
                };

                FtpLogger.LogInfo("=== CONSENT INFO REQUEST COMPLETED SUCCESSFULLY ===");
            ...
            {
                FtpLogger.LogError("================================================================================");
                FtpLogger.LogError(string.Format("=== CONSENT INFO REQUEST FAILED - RequestId: {0} ===", requestId));
                FtpLogger.LogError("================================================================================");
                FtpLogger.LogError(string.Format("Exception Type: {0}", ex.GetType().FullName));
                FtpLogger.LogError(string.Format("Exception Message: {0}", ex.Message));
                FtpLogger.LogError(string.Format("Stack Trace: {0}", ex.StackTrace));
                ...
                return Request.CreateResponse(HttpStatusCode.InternalServerError, 
                    ...
                    { 
                        Success = false, 
                        Message = "An error occurred while retrieving consent document information",
                        Error = ex.Message,
                        Data = new
                        {
                            hint = "Check detailed logs at: C:\\smkcapi_published\\Logs\\FtpLog_YYYYMMDD.txt",
        ...
        /// <summary>
        /// Upload/Create a new consent document
        ...
        public HttpResponseMessage UploadConsentDocument([FromBody] UploadConsentDocumentRequest request)
        {
            ...
            try
            {
                // Log request start
                FtpLogger.LogInfo("================================================================================");
                FtpLogger.LogInfo(string.Format("=== CONSENT UPLOAD REQUEST START - RequestId: {0} ===", requestId));
                FtpLogger.LogInfo("================================================================================");
                FtpLogger.LogInfo(string.Format("Client IP: {0}", clientIp));
                FtpLogger.LogInfo(string.Format("RequestId: {0}", requestId));
                FtpLogger.LogInfo(string.Format("Timestamp: {0:yyyy-MM-dd HH:mm:ss.fff} UTC", DateTime.UtcNow));
                ...
                FtpLogger.LogStep(2, "Preparing to upload file to storage", true);
                ...
                FtpLogger.LogStep(3, "Calling Storage Service", true,
                    string.Format("Uploading: {0} for {1}/{2}", consentDocument.FileName, request.RequirementId, request.BankId));

                var uploadedFileName = _ftpStorage.UploadConsentDocument(
                ...
                FtpLogger.LogStep(3, "Storage Service completed", true,
                ...
                FtpLogger.LogInfo("=== CONSENT UPLOAD REQUEST COMPLETED SUCCESSFULLY ===");
            ...
            {
                FtpLogger.LogError("================================================================================");
                FtpLogger.LogError(string.Format("=== CONSENT UPLOAD REQUEST FAILED (NULL ARGUMENT) - RequestId: {0} ===", requestId));
                FtpLogger.LogError("================================================================================");
            ...
            {
                FtpLogger.LogError("================================================================================");
                FtpLogger.LogError(string.Format("=== CONSENT UPLOAD REQUEST FAILED (INVALID ARGUMENT) - RequestId: {0} ===", requestId));
                FtpLogger.LogError("================================================================================");
                FtpLogger.LogError(string.Format("Parameter: {0}", argEx.ParamName));
                FtpLogger.LogError(string.Format("Message: {0}", argEx.Message));
                FtpLogger.LogError("================================================================================");
            ...
            {
                FtpLogger.LogError("================================================================================");
                FtpLogger.LogError(string.Format("=== CONSENT UPLOAD REQUEST FAILED (INVALID OPERATION) - RequestId: {0} ===", requestId));
                FtpLogger.LogError("================================================================================");
            ...
            {
                FtpLogger.LogError("================================================================================");
                FtpLogger.LogError(string.Format("=== CONSENT UPLOAD REQUEST FAILED - RequestId: {0} ===", requestId));
                FtpLogger.LogError("================================================================================");
                FtpLogger.LogError(string.Format("Exception Type: {0}", ex.GetType().FullName));
                FtpLogger.LogError(string.Format("Exception Message: {0}", ex.Message));
                FtpLogger.LogError(string.Format("Stack Trace: {0}", ex.StackTrace));
        ...
        private void LogRequest(string action, bool success)
        {
            ...
            var logEntry = string.Format("{0:yyyy-MM-dd HH:mm:ss} UTC - CONSENT_CONTROLLER_{1} - Status: {2}, ClientIP: {3}, RequestId: {4}",
                DateTime.UtcNow,
                action.ToUpper(),
        ...
        {
            var logEntry = string.Format("{0:yyyy-MM-dd HH:mm:ss} UTC - CONSENT_CONTROLLER_{1}_ERROR: {2}",
                DateTime.UtcNow,
                action.ToUpper(),
                ex.Message);
            ...
            var clientIp = GetClientIpAddress();
            var securityLogEntry = string.Format("{0:yyyy-MM-dd HH:mm:ss} UTC - SECURITY_ERROR - Controller: ConsentDocumentController, Action: {1}, ClientIP: {2}, Error: {3}",
                DateTime.UtcNow,
                action,
                clientIp,
                ex.Message);
